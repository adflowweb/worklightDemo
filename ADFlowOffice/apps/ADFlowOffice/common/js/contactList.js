...
ADF.view.ContactList = Backbone.View.extend({
...
//				var temp = true;
//				var liSrc = '';
//				for (var j = 0; j < this.listMax; j++) {
//
//					temp = !temp;
//
//					var temp2 = temp ? "1" : "2";
//					console.log(temp2);
//					liSrc += '<li id="'+ this.collection.models[j].get('no') +'" class="addressListLi' + temp2
...
//					displayloadDeliveryList(result.invocationResult.array);
					...
					console.log("===============   items[0].no ==============" + items[0].no);
					for ( var i = 0; i < items.length ; i++) {
				    	this.collection
						.add(new ADF.model.ContactItem(
								{
									nameen : items[i].nameen,
									hiredate : items[i].hiredate,
									sex : items[i].sex,
									phone : items[i].phone,
									email : items[i].email,
									no : items[i].no,
				 	...
				 	console.log("===============   this.collection.models[j].get('no') ==============" + this.collection.models[0].get('no'));
